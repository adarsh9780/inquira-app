...
test('public site links to an honest distribution doc and the doc explains the unsigned installer tradeoff', () => {
  ...
  assert.match(landing.download.dialog.steps.join('\n'), /asks before removing Apple's quarantine flag/)
  ...
  assert.match(distributionDoc, /GitHub builds macOS/)
  assert.match(distributionDoc, /Cloudflare R2/)
  assert.match(distributionDoc, /source repository is private/)
  assert.match(distributionDoc, /paid developer\s+program/i)
  assert.equal(landing.download.dialog.futurePlanLink.href, '/docs/getting-started/distribution#macos')
  assert.match(distributionDoc, /curl -fsSL https:\/\/inquiraai\.com\/install\.sh \| bash/)
  assert.match(installerScript, /manifest_value macos_arm64_sha256/)
  assert.match(installerScript, /shasum -a 256/)
  assert.doesNotMatch(installerScript, /brew install/)
  assert.match(installerScript, /xattr -dr com\.apple\.quarantine/)
  assert.match(installerScript, /Remove the quarantine attribute/)
})
